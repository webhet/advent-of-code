...
fn main() {
    ...
    for _ in 0..1000 {
        let (lp, hp) = compute_state(&broadcast_outputs, &mut modules);
        low_pulse_count += lp;
        high_pulse_count += hp;
    }

    println!("Pulses: {}", low_pulse_count * high_pulse_count);
}
...
fn compute_state(
    ...
    let mut low_pulse_count = 1; // The pulse from the button to the broadcaster
    let mut high_pulse_count = 0;
